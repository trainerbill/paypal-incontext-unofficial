    ...
    paypal.checkout = function () {
        ...
        function _track(msg) {
            ...
            if (config.log && typeof msg === "object") {
                ...
                //beacon.src = url + '/webapps/hermes/api/log?event=' + msg.step + '&state='+ msg.status+'merchantID=' + config.merchantID + '&cb='+Date.now()+'&msg=' + msgStr;
        ...
        function _destroy(e) {
            ...
            if (isOpen && UI.wrapper && UI.wrapper.parentNode) {
                ...
                if (msg && config.cancelUrl) {
                    ...
                    UI.wrapper.parentNode.removeChild(UI.wrapper);
                    //document.body.classList.remove(config.name);
        ...
        /**
         * Create an iFrame Bridge to communicate between Mini browser and parent window.
         * This is only for IE
         * @returns {undefined}
         */
        function _setUpPayPalBridge() {
            ...
            if(config.devMode) {
                iframe.src = 'http://localhost:9000/webapps/hermes/bridge';
            }
        ...
        function _init() {
            ...
            //Make a call to PXP to get in-context vs Full page experience - this is only for pilot
            //_pxpDecision();
        ...
        function _getBtnContainers(el, result) {
            ...
            if (el.constructor.toString().indexOf('Array') > -1 || ((el.length > 0) && (typeof el.item !== 'undefined'))) {
                ...
                if(domEl) {
                    ...
                    _track({"step": "SETUP", "status": "ERROR", "error_msg": "Can't find element " + el});
                }
            }
        }
        ...
        function _initWithButtonGeneration () {
            ...
            if(customBtnIds){
                var staticBtns = document.querySelectorAll("." + config.staticBtnHiddenStyle);
                var staticBtnsLength = staticBtns.length;
                for(var i=0; i<staticBtnsLength; i++) {
                    staticBtns[i].className = staticBtns[i].className.replace(config.staticBtnHiddenStyle, '');
                }
            }
        }
        ...
        app.setup = function (merchantId, merchantConfig) {
            ...
            if(!merchantId) {
                _track({"step": "SETUP", "status": "ERROR", "error_msg": "Merchant id is not provided for setup!"});
            ...
            if(merchantConfig && merchantConfig.button) {
        ...
        app.startFlow = function (url) {
            ...
            if (url && url.toLowerCase().indexOf("ec-") === 0) {
                url = config.sandBox ? app.sandBoxUrlPrefix + url : app.liveUrlPrefix + url;
